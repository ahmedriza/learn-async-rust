...
pub const FN_KW: &str = "coroutine";
pub const W_KW: &str = "wait";
pub static L_TRM_LN: OnceCell<usize> = OnceCell::new();
...
pub fn rewrite(src: String, dest: File) -> anyhow::Result<()> {
    detect_line_ending(&src);
    let mut dest = dest;
    // Find start point of async blocks
    let start_points = find_kw_start_points(&src);

    // No keywords, no async functions, do nothing
    if start_points.is_empty() {
        return Err(anyhow::anyhow!("No `{FN_KW}` function found."));
    }
...
fn find_kw_start_points(src: &str) -> Vec<usize> {
    let mut start_points = Vec::new();
    let mut index = 0;

    for line in src.lines() {
        // Remove everything that is commented out
        let (txt, _) = match line.split_once("//") {
            Some((txt, commented)) => (txt, commented),
            None => (line, ""),
        };
        // Search in the text that's not commented out
...
fn create_new_async_fn(
    ...
    let (fn_name, arg_start) =
        def.split_once("(").expect("Expected `(`").clone();
...
fn format_args_types_only(args: &[(String, String)]) -> String {
    if args.is_empty() {
        ...
        let mut args_fmt: String =
            args.iter().map(|(_n, ty)| format!("{ty},")).collect();
        // remove last `,`
        args_fmt.pop();
        format!("({args_fmt})")
    }
...
fn format_args_name_and_types(args: &[(String, String)]) -> String {
    if args.is_empty() {
        ...
        let mut args_fmt: String =
            args.iter().map(|(n, ty)| format!("{n}: {ty},")).collect();
        // remove last `,`
        args_fmt.pop();
        format!("{args_fmt}")
    }
...
fn format_args_names_only(args: &[(String, String)]) -> String {
    if args.is_empty() {
        ...
        let mut args_fmt: String =
            args.iter().map(|(n, _ty)| format!("{n},")).collect();
        // remove last `,`
        args_fmt.pop();
        format!("({args_fmt})")
    }
...
pub fn detect_line_ending(s: &str) {
    ...
    let len = loop {
        ...
        match chars.peek() {
            ...
            Some(_) => (),
            None => {
                // default if for some reason we fall through
                if cfg!(windows) {
                    break 2;
                } else {
                    break 1;
